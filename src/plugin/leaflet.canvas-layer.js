...
function layerFactory(L) {
    var CanvasLayer = (L.Layer ? L.Layer : L.Class).extend({
        ...
        _getClassName() {
            return this.options && this.options.className
                ? this.options.className
                : null;
        },

        _animateZoom: function (event) {
            if (event) this._updateCanvasPosition(event.center, event.zoom);
        },

        // Resets canvas position. Fixes the issue of wrong markers positioning after repositioning (panning) the map
        _updateCanvasPosition(center, zoom) {
        ...
        _drawMarker: function (marker, pointPos) {
            ...
            if (!this._imageLookup)
                // Create an object to store images and markers linked to them
                // Normally, there is only one image used to draw all the markers,
                // But also may be a separate image for each marker (first draw will be very slow in this case)
                this._imageLookup = {};
            ...
            if (
                this.options.userDrawFunc &&
                typeof this.options.userDrawFunc === "function"
            ) {
                // Draw marker using custom draw function `userDrawFunc`
                const size = marker.options.icon.options.iconSize;
                this.options.userDrawFunc(this, marker, pointPos, size);
            } else if (marker.canvas_img) {
                // the marker already has loaded image, draw it
                self._drawImage(marker, pointPos);
            } else {
                // No loaded image for the marker. We need to loaded it
                // Let's check if icon url is provided
                let iconUrl = marker.options.icon.options.iconUrl;
                let isIcon = true;
                if (!iconUrl) {
                    // Identify marker's image by type if provided or by the image html itself
                    // Type should be provided by a user and be unique for each unique marker image
                    iconUrl =
                        marker.options.type || marker.options.icon.options.html;
                    if (!iconUrl) {
                        // impossible, but still
                        console.error("No image specified for the marker");
                        return;
                    }
                    isIcon = false; // Marker image is specified in html, not icon prop of the icon
                }

                // Check if the image of this type already has been loaded and saved
                if (self._imageLookup[iconUrl]) {
                    marker.canvas_img = self._imageLookup[iconUrl][0];
                    if (self._imageLookup[iconUrl][1] === false) {
                        // the image is not loaded yet. Just save the marker. It will be drawn as soon as the image has been loaded
                    ...
                    var i = new Image();
                    if (isIcon === true) {
												// Marker's icon is specified as a simple icon by path
                        i.src = iconUrl;
                        ...
                        i.onload = function () {
                            ...
                            self._imageLookup[iconUrl][2].forEach(function (e) {
                                self._drawImage(e[0], e[1]);
												...
												// Marker's icon is specified as an image by html (svg)
                        let domObj = window.URL || window.webkitURL || window;
                        let url = domObj.createObjectURL(
                            new Blob([marker.options.icon.options.html], {
                                type: "image/svg+xml",
                            })
                        );
                        i.src = url;
        ...
        _initCanvas: function () {
            this._canvas = L.DomUtil.create(
                "canvas",
                "leaflet-canvas-layer leaflet-layer ".concat(
                    this._getClassName()
                )
            );
